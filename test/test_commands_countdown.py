...
async def test_current_countdown_no_cog(mock_interaction, mock_hunt_bot, mock_bot):
    ...
    mock_interaction.response.send_message.assert_called_with("CountdownCog is not available or misconfigured.",
                                                              ephemeral=True)
...
async def test_current_countdown_before_start(mock_interaction, mock_hunt_bot, mock_bot):
    ...
    mock_hunt_bot.start_datetime = now + timedelta(minutes=11)
...
def test_register_countdown_commands_adds_command():
    ...
    tree.command.assert_called_once_with(name="countdown",
                                         description="Show time remaining until the Hunt starts or ends")
